module ContentManager.Utils.Reflection
...
let GetPropertyName (funcExpr: Expr<'T -> 'U>) =
    match funcExpr with
    | Lambda (_, PropertyGet (_, fi, _)) -> fi.Name
    | _ -> failwith $"Error: method not exist {funcExpr.Raw}"